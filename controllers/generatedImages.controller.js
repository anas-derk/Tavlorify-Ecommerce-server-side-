    ...
    try{
        ...
        switch (result.data.modelName) {
            case "dreamshaper": {
                ...
                {
                    prompt: `${textAfterTranslation}, ${result.data.categoryName}, ${result.data.prompt}`,
...
async function generateImageUsingFaceSwapService(req, res) {
    ...
    if (generatedImagePathInServer) {
        await saveNewGeneratedImageDataGlobalFunc({ service: "face-swap", imageLink, categoryName: result.data.categoryName, paintingType, position, dimentionsInCm, isExistWhiteBorder, frameColor, generatedImageURL: generatedImagePathInServer }, generatedImageAsArrayBuffer);
    }
}
